...
function printMovieTitles(movies: Movie[]): void {
  // write your code here...
  movies.map((n) => console.log(n.title));
}
...
function countMoviesByYear(movies: Movie[], year: number): number {
  // write your code here...
  const totalMovies: Movie[] = movies.filter((movie) => movie.year === year);
  return totalMovies.length; // replace -1 with what you see is fit
}
